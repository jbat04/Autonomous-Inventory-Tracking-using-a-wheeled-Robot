...
extern void gps_location(loc_t *coord, int port) {
    uint8_t status = _EMPTY;
    while(status != _COMPLETED) {
        gpgga_t gpgga;
        gprmc_t gprmc;
        char buffer[256];// 255 is the max value of an uint8_t

        serial_readln(buffer, 256, port);
	if(strlen(buffer) > 10){
